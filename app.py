...
def show_input_step2():
    ...
    # ✅ แก้ไขปัญหากล่องตัวเลือก: ใส่ค่าให้ครบถ้วนป้องกัน Error
    score_options = [1-5]
    ...
    with st.form("form_step2"):
        ...
        if submitted:
            ...
            # เรียกใช้งาน AI ประมวลผล
            success = process_results()
            
            # ✅ เปลี่ยนหน้าเฉพาะเมื่อการคำนวณผ่าน 100% (ถ้า Error จะหยุดและโชว์ตัวแดงให้เห็น)
            if success:
...
def process_results():
    ...
    if predictor_model is not None and not df_raw.empty:
        try:
            # 2.1 ✅ ใช้วิธีแม่แบบ (Template): ดึงข้อมูลแถวแรกของ Excel มาเป็นฐาน จะได้ไม่มีปัญหาเรื่องชนิดข้อมูล
            pred_df = df_raw.iloc[0:1].copy().reset_index(drop=True)
            
            # 2.2 ✅ เอาคำตอบของคุณไปทับในแม่แบบ (ข้อไหนไม่ถาม ปล่อยเป็นข้อมูลจริงตาม Excel)
            for key, val in inputs.items():
                if key in pred_df.columns:
                    pred_df.at[0, key] = float(val) if isinstance(val, (int, float)) else val

            # 2.3 ใส่ค่าตัวแปรพื้นฐานตามงานวิจัย
            if 'SIZ' in pred_df.columns: pred_df.at[0, 'SIZ'] = 1 
            if 'YER' in pred_df.columns: pred_df.at[0, 'YER'] = 10 
                
            # 2.4 สั่งพยากรณ์ผลลัพธ์
            prob_df = predictor_model.predict_proba(pred_df)
            
            # 2.5 ดึงค่าคะแนนความน่าจะเป็นของ Class 1 อย่างปลอดภัยสูงสุด
            ...
            # 🛑 หากมี Error จะหยุดอยู่หน้านี้ และโชว์กล่องแดงให้รู้สาเหตุทันที
