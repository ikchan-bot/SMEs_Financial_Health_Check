...
# --- หน้าที่ 4: Dashboard (Result) - แก้ไข Logic ให้คะแนนสัมพันธ์กับ DNA 100% ---
def show_dashboard():
    # 1. ฝัง CSS
    st.markdown("""
        <style>
    ...
    try:
        ...
        if 'scaler_model' in globals() and 'kmeans_model' in globals():
            ...
            cluster_id = 0 # Default Potential
    except:
        cluster_id = 0

    # --- 2.2 คำนวณความเสี่ยง (Risk Prediction) ---
    # เราจะหาค่า Probability ดิบ (0.0 - 1.0) มาก่อน แล้วค่อย map เข้าช่วง
    raw_prob = 0.5 # ค่าเริ่มต้นกลางๆ
    ...
    try:
        if 'predictor_model' in globals() and predictor_model is not None:
            ...
            # ดึงค่าความเสี่ยงดิบ (0.0 - 1.0)
            proba_df = predictor_model.predict_proba(pred_df)
            
            if 1 in proba_df.columns:
                raw_prob = proba_df[1].iloc[0] 
            else:
                raw_prob = proba_df.iloc[0, 1]
        else:
            # ถ้าไม่มี Model ให้คำนวณมือคร่าวๆ
    ...
    # =========================================================
    # ✅✅✅ จุดแก้ไขสำคัญ (Force Logic) ✅✅✅
    # บังคับค่า Risk Score ให้ตรงกับ Cluster เสมอ (Normalization)
    # =========================================================
    
    # กำหนด Mapping ตามที่คุณต้องการ:
    # 0: Potential Starter (แดง) -> ต้อง 71-100%
    # 1: Active Marketer (เหลือง) -> ต้อง 41-70%
    # 2: Master Leader (เขียว) -> ต้อง 0-40%
    
    if cluster_id == 0: # Potential Starter (High Risk)
        # แปลงค่า 0.0-1.0 ให้ไปตกในช่วง 71-100
        # สูตร: 71 + (raw_prob * 29) -> ผลลัพธ์จะเป็น 71 ถึง 100
        risk_score = 71 + (raw_prob * 29)
        
    elif cluster_id == 1: # Active Marketer (Medium Risk)
        # แปลงค่า 0.0-1.0 ให้ไปตกในช่วง 41-70
        risk_score = 41 + (raw_prob * 29)
        
    else: # cluster_id == 2: Master Leader (Low Risk)
        # แปลงค่า 0.0-1.0 ให้ไปตกในช่วง 0-40
        risk_score = raw_prob * 40

    # ปัดทศนิยม
    risk_score = min(100, max(0, risk_score)) # กันค่าเกิน
