...
def show_input_step1():
    # 1. ฝัง CSS ปรับแต่งปุ่มกด (Chula Style)
    # ปกติ: สีเทา (#A9A9A9) | Hover: สีชมพูจุฬา (#FF5C8D)
    st.markdown("""
        <style>
        /* เข้าถึงปุ่ม Primary ภายใน Form */
        div[data-testid="stForm"] button[kind="primary"] {
            background-color: #A9A9A9 !important; /* สีเทา */
            border-color: #A9A9A9 !important;
            color: white !important;
            border-radius: 8px;
            transition: 0.3s; /* เอฟเฟกต์เปลี่ยนสีนุ่มนวล */
        }
        /* เมื่อเอาเมาส์ไปวาง (Hover) */
        div[data-testid="stForm"] button[kind="primary"]:hover {
            background-color: #FF5C8D !important; /* สีชมพูจุฬา (Chula Pink) */
            border-color: #FF5C8D !important;
            box-shadow: 0 4px 8px rgba(255, 92, 141, 0.4);
        }
        </style>
    """, unsafe_allow_html=True)
    ...
    with st.form("form_step1"):
        ...
        # ปุ่ม Submit (จะถูกเปลี่ยนสีด้วย CSS ด้านบน)
...
def show_input_step2():
    ...
    with st.form("form_step2"):
        ...
        with col1:
            ...
            cap_netw = st.selectbox("ท่านใช้เครือข่ายหรือพันธมิตรในการดำเนินธุรกิจมากเพียงใด (CAP_NETW)", score_options, index=0)
        ...
        with col2:
            ...
            prc_cfw = st.selectbox("กระแสเงินสดเพื่อประกอบธุรกิจและชำระหนี้อยู่ในระดับใด (PRC_CFW)", score_options, index=0)
        ...
        with col3:            
            ...
            ecm_net = st.selectbox("การเข้าถึงเครือข่ายอินเตอร์เน็ตของกิจการอยู่ในระดับใด (ECM_NET)", score_options, index=0)
            res_ch = st.selectbox("ความสามารถในการโต้ตอบลูกค้าผ่านช่องทางต่าง ๆ อยู่ในระดับใด (RES_CH)", score_options, index=0)
    ...
    with col2:
        ...
        gauge = {
            ...
            'steps': [
                ...
                {'range': , 'color': "#2ecc71"},   # สีเขียว (0-40 คะแนน)
                {'range': , 'color': "#f1c40f"},  # สีเหลือง (40-70 คะแนน)
                {'range': , 'color': "#e74c3c"}  # สีแดง (70-100 คะแนน)
            ],
            'threshold': {
