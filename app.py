...
# --- หน้าที่ 4: Dashboard (Result) - แก้ไข Logic บังคับช่วงคะแนนให้ตรงกลุ่ม ---
def show_dashboard():
    # 1. ฝัง CSS
    st.markdown("""
        <style>
    ...
    # หาค่าดิบ (0.0 - 1.0) มาก่อน แล้วค่อยเอาไป Map เข้าช่วง
    raw_prob = 0.5 
    ...
    # เพื่อให้ DNA กับ กราฟ แสดงผลสอดคล้องกัน 100%
    # =========================================================
    
    if cluster_id == 0: # Potential Starter (แดง) -> บังคับช่วง 71-100%
        # สูตร: ฐาน 71 + (ค่าดิบ * ช่องว่าง 29)
        risk_score = 71 + (raw_prob * 29)
        
    elif cluster_id == 1: # Active Marketer (เหลือง) -> บังคับช่วง 41-70%
        # สูตร: ฐาน 41 + (ค่าดิบ * ช่องว่าง 29)
        risk_score = 41 + (raw_prob * 29)
        
    else: # Master Leader (เขียว) -> บังคับช่วง 0-40%
    ...
    with col2:
        ...
        fig = go.Figure(go.Indicator(
            ...
            gauge = {
                ...
                'bordercolor': "gray",
                'steps': [
                    {'range': [0, 40], 'color': "#2ecc71"},   # เขียว (Master)
                    {'range': [40, 70], 'color': "#F9D607"},  # เหลือง (Active)
                    {'range': [70, 100], 'color': "#e74c3c"}  # แดง (Potential)
                ],
                'threshold': {
