...
def show_dashboard():
    # ตรวจสอบว่ามีข้อมูลหรือไม่ (ป้องกัน Error กรณีข้ามหน้า)
    if 'inputs' not in st.session_state or not st.session_state.inputs:
        st.warning("⚠️ กรุณากรอกข้อมูลในขั้นตอนที่ 1 และ 2 ก่อนครับ")
    ...
    # --- ส่วนที่ 1: ประมวลผล K-Means (DNA) ---
    # เตรียมข้อมูลสำหรับ Clustering (ต้องเรียงลำดับ feature ให้ตรงกับตอน Train โมเดล)
    # สมมติลำดับคือ: BEH_MON, BRN_IMAGE, BRN_BRAND, SAV_VIRUS, SAV_PDPA, CRI_PLN, POL_BEN, POL_ADJ
    cluster_features = [
        inputs.get('BEH_MON', 3), inputs.get('BRN_IMAGE', 3), inputs.get('BRN_BRAND', 3),
        inputs.get('SAV_VIRUS', 3), inputs.get('SAV_PDPA', 1), inputs.get('CRI_PLN', 2),
        inputs.get('POL_BEN', 2), inputs.get('POL_ADJ', 3)
    ]
    
    # ทำนายกลุ่ม (Cluster)
    # หมายเหตุ: ในโค้ดจริงต้องโหลด scaler และ kmeans_model มาก่อน (ดูใน load_resources)
    # ตรงนี้ผมใส่ Dummy Logic ไว้ก่อนเพื่อให้รันผ่าน (ท่านต้องใช้โค้ดจริงบรรทัดที่มี kmeans_model.predict)
    try:
        # data_scaled = scaler_model.transform([cluster_features])
        # cluster_id = kmeans_model.predict(data_scaled)
        cluster_id = 0 # (สมมติว่าเป็นกลุ่ม 0 ถ้ายังไม่ได้โหลดโมเดลจริง)
    except:
        cluster_id = 0 # กรณี Error ให้ Default เป็น 0

    # ชื่อกลุ่มและคำแนะนำ (ตามงานวิจัยของท่าน)
    cluster_info = {
        0: {"name": "Active Marketer (นักการตลาดไฟแรง)", "desc": "เก่งการขาย การตลาดและการสร้างแบรนด์ แต่ขาดเทคโนโลยีและการป้องกันความเสี่ยง", "color": "#f1c40f"}, # สีเหลือง
        1: {"name": "Potential Starter (นักสู้ผู้มีศักยภาพ)", "desc": "ท่านต้องเสริมความพร้อมด้านการเงิน การตลาด เทคโนโลยีและการจัดการ", "color": "#e74c3c"},   # สีแดง
        2: {"name": "Master Leader (ผู้นำระดับมาสเตอร์)", "desc": "ความสามารถรอบด้าน มีความพร้อมด้านการเงิน การตลาด เทคโนโลยีและการจัดการความเสี่ยง", "color": "#2ecc71"}    # สีเขียว
    }
    dna_result = cluster_info.get(cluster_id, cluster_info)

    # --- ส่วนที่ 2: ประมวลผล AutoGluon (ความเสี่ยง) ---
    # (ตรงนี้ต้องใช้ predictor_model.predict_proba ในโค้ดจริง)
    # สมมติค่าความเสี่ยงที่ได้จากการพยากรณ์ (0.0 - 1.0)
    risk_score = 0.75 # สมมติว่าเสี่ยง 75% (ท่านต้องเอาค่า prob จากโมเดลมาใส่แทน)
    risk_percent = risk_score * 100

    # --- แสดงผลหน้าจอ (Dashboard UI) ---
    st.markdown(f"<h2 style='text-align: center; color: #1E3A8A;'>ผลการประเมินสุขภาพการเงินและ DNA ธุรกิจ</h2>", unsafe_allow_html=True)
    st.markdown("---")
    ...
    with col1:
        ...
        st.markdown(f"<div style='background-color: {dna_result['color']}; padding: 20px; border-radius: 10px; color: white; text-align: center;'>"
                    f"<h2>{dna_result['name']}</h2>"
                    f"<p>{dna_result['desc']}</p>"
                    f"</div>", unsafe_allow_html=True)
        st.markdown("---")
        st.markdown("**คำแนะนำเบื้องต้น:**")
        if cluster_id == 1:
            st.warning("⚠️ ธุรกิจของท่านมีความยืดหยุ่น อ่อนไหว ควรเริ่มวางระบบบัญชีและจัดการด้านการเงินโดยด่วน")
        elif cluster_id == 0:
            st.info("ℹ️ ท่านมีจุดแข็งด้านการตลาด แต่ควรเพิ่มความปลอดภัยทางเทคโนโลยีและระบบจัดการความเสี่ยงเพื่อความยั่งยืน")
        else:
            st.success("✅ ท่านมีความพร้อมสูง มีโอกาสเข้าถึงแหล่งเงินทุนและยื่นขอสินเชื่อเพื่อขยายกิจการได้เลย")

    with col2:
        st.markdown("### 🔮 ระดับความเสี่ยง (Risk Score)")
        
        # --- แก้ไขกราฟ Gauge (เข็มไมล์) ให้ถูกต้อง ---
        fig = go.Figure(go.Indicator(
            mode = "gauge+number",
            value = risk_percent,
            domain = {'x': [1], 'y': [1]}, # กำหนด Domain ให้เต็มพื้นที่
            title = {'text': "โอกาสเข้าถึงแหล่งเงินทุน (คะแนนความเสี่ยง)", 'font': {'size': 20}},
            gauge = {
                ...
                'bordercolor': "gray",
                'steps': [
                    {'range': [2], 'color': "#2ecc71"},  # เขียว (เสี่ยงต่ำ/โอกาสสูง)
                    {'range': [2, 3], 'color': "#f1c40f"}, # เหลือง (ปานกลาง)
                    {'range': [3, 4], 'color': "#e74c3c"} # แดง (เสี่ยงสูง/โอกาสต่ำ)
                ],
                'threshold': {
    ...
    # ปุ่มดำเนินการต่อ
    st.markdown("---")
    col_btn1, col_btn2, col_btn3 = st.columns([1, 5])
    with col_btn2:
        if st.button("📄 ดูข้อเสนอแนะโดยละเอียด (Recommendation)", type="primary", use_container_width=True):
            navigate_to('recommendation') # ต้องสร้างหน้านี้เพิ่ม หรือลิงก์ไปหน้าอื่น
