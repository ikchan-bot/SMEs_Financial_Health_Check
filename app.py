...
def show_landing():
    ...
    with col2:
        ...
        st.markdown("---")
        st.caption("พัฒนาโดย: นายสมเกียรติ จูสวัสดิ์ | หลักสูตรปริญญาเอก ธุรกิจเทคโนโลยีและการจัดการนวัตกรรม จุฬาลงกรณ์มหาวิทยาลัย")
...
def show_input_step1():
    ...
    with st.form("form_step1"):
        ...
        with col1:
            ...
            beh_mon = st.selectbox("ท่านให้ความสำคัญกับวินัยและการติดตามตรวจสอบการเงิน (BEH_MON)", [1-5], index=3)
            brn_image = st.selectbox("ท่านให้ความสำคัญกับภาพลักษณ์องค์กร (BRN_IMAGE)", [1-5], index=3)
            brn_brand = st.selectbox("การรับรู้และความน่าเชื่อถือของแบรนด์ของท่าน (BRN_BRAND)", [1-5], index=3)
        
        with col2:
            st.markdown("**ด้านเทคโนโลยีและนโยบาย**")
            sav_virus = st.selectbox("การอัพเดทโปรแกรมป้องกันไวรัสเพื่อความปลอดภัย (SAV_VIRUS)", [1-5], index=3)
            sav_pdpa = st.selectbox("ท่านปฏิบัติตามกฎหมาย PDPA (SAV_PDPA)", [1-5], index=1)
            cri_pln = st.selectbox("ท่านมีแผนรองรับวิกฤตการณ์/ซ้อมหนีไฟ (CRI_PLN)", [1-5], index=2)
            pol_ben = st.selectbox("ท่านได้รับประโยชน์จากนโยบายภาครัฐ (POL_BEN)", [1-5], index=2)
            pol_adj = st.selectbox("ท่านสามารถปรับรูปแบบธุรกิจให้สอดคล้องนโยบายรัฐ (POL_ADJ)", [1-5], index=3)
