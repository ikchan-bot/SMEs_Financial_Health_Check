...
def show_input_step2():
    ...
    with st.form("form_step2"):
        ...
        with col1:
            st.markdown("**การเงินและเครือข่าย**")
            prc_cfw = st.selectbox("กระแสเงินสดเพื่อประกอบธุรกิจและชำระหนี้อยู่ในระดับใด (PRC_CFW)", score_options, index=3)
            cap_netw = st.selectbox("ท่านใช้เครือข่ายหรือพันธมิตรในการดำเนินธุรกิจ (CAP_NETW)", score_options, index=3)
            eco_adt = st.selectbox("ความสามารถในการปรับตัวรับสถานการณ์เศรษฐกิจ (ECO_ADT)", score_options, index=3)

        with col2:
            st.markdown("**เทคโนโลยีและการสื่อสาร**")
            ecm_net = st.selectbox("การเข้าถึงเครือข่ายอินเตอร์เน็ตของกิจการ (ECM_NET)", score_options, index=3)
            res_ch = st.selectbox("ความสามารถในการโต้ตอบลูกค้าผ่านช่องทางต่างๆ (RES_CH)", score_options, index=3)
            tmc_live = st.selectbox("ทักษะการ Live ขายสินค้า (TMC_LIVE)", score_options, index=2)

        with col3:
            st.markdown("**นโยบายองค์กร**")
...
def show_dashboard():
    ...
    with col2:
        ...
        # กำหนดสีตามความเสี่ยง (เขียว=เสี่ยงต่ำ/ข้อจำกัดการเข้าถึงแหล่งเงินทุนต่ำ, แดง=เสี่ยงสูง/ข้อจำกัดการเข้าถึงแหล่งเงินทุนสูง)
...
def show_profile():
    st.markdown("## 👤 โปรไฟล์")
    st.write("เพื่อให้งานวิจัยนี้สมบูรณ์ โปรดบันทึกข้อมูลเพื่อการอ้างอิงและการบริจาค")
