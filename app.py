...
def show_input_step2():
    ...
    st.markdown("### 💼 การจัดการธุรกิจ")
    st.info("💡 โปรดตอบคำถาม 0 = ไม่มี, 1 = น้อยที่สุด, 5 = มากที่สุด")

    with st.form("form_step2"):
        col1, col2 = st.columns(2)
        with col1:
            prc_cfw = st.slider("กระแสเงินสดเพื่อประกอบธุรกิจและชำระหนี้อยู่ในระดับใด (PRC_CFW)", 0, 5, 3)
            cap_netw = st.slider("ท่านใช้เครือข่ายหรือพันธมิตรในการดำเนินธุรกิจ (CAP_NETW)", 0, 5, 3)
        ...
        with col2:
            ...
            csr3 = st.radio("กิจการของท่านมีระบบกำจัดของเสีย (CSR3)", ["ไม่มี (0)", "มี (1)"])
            ohr_career = st.radio("กิจการของท่านมีเส้นทางอาชีพให้พนักงานรับรู้ (OHR_CAREER)", ["ไม่มี (0)", "มี (1)"])
