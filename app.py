...
def show_dashboard():
    ...
    cluster_info = {
        0: {"name": "Active Marketer (นักการตลาดไฟแรง)", "desc": "โดดเด่นด้านการตลาดและการสร้างแบรนด์ แต่ต้องระวังระบบงานหลังบ้าน", "color": "#3498db"},
        1: {"name": "Potential Starter (นักสู้ผู้มีศักยภาพ)", "desc": "อยู่ในช่วงเริ่มต้นสร้างรากฐาน จำเป็นต้องเสริมสร้างวินัยการเงิน", "color": "#f1c40f"},
    ...
    with col2:
        ...
        # กำหนดสีตามความเสี่ยง (เขียว=เสี่ยงต่ำ/ความเสี่ยงด้านแหล่งเงินทุนต่ำ, แดง=เสี่ยงสูง/ความเสี่ยงด้านแหล่งเงินทุนสูง)
        # Model Output: 1 = มีข้อจำกัด (เสี่ยงสูง), 0 = ไม่มีข้อจำกัด (เสี่ยงต่ำ)
        # ดังนั้น Score สูง = สีแดง
        if risk_score < 40:
            risk_color = "green"
            risk_text = "ความเสี่ยงด้านแหล่งเงินทุนต่ำ (Low Risk)"
        elif risk_score < 70:
            risk_color = "orange"
            risk_text = "ความเสี่ยงด้านแหล่งเงินทุนปานกลาง (Moderate Risk)"
        else:
            risk_color = "red"
            risk_text = "ความเสี่ยงด้านแหล่งเงินทุนสูง (High Risk)"

        st.markdown(f"### 🔮 ระดับความเสี่ยงการเข้าถึงแหล่งเงินทุน: **{risk_score:.1f}%**")
