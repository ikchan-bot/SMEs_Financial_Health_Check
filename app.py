...
# --- หน้าที่ 4: Dashboard (Result) - แก้ไข Mapping Cluster ให้ถูกต้องตามผลวิเคราะห์ ---
def show_dashboard():
    # 1. ฝัง CSS
    st.markdown("""
        <style>
    ...
    try:
        ...
        if 'scaler_model' in globals() and 'kmeans_model' in globals():
            ...
            cluster_id = 1 # Default เป็น 1 (Potential) เพื่อความปลอดภัยถ้าโมเดลไม่โหลด
    except:
        cluster_id = 1
    ...
    try:
        if 'predictor_model' in globals() and predictor_model is not None:
            ...
            # ดึงค่าความเสี่ยงดิบจาก Model
            proba_df = predictor_model.predict_proba(pred_df)
            if 1 in proba_df.columns:
                raw_prob = proba_df[3].iloc
            else:
                raw_prob = proba_df.iloc[3]
        else:
            # คำนวณมือถ้าไม่มี Model
    ...
    # =========================================================
    # ✅✅✅ แก้ไข: บังคับช่วงคะแนนตามกลุ่ม (Score Normalization) ✅✅✅
    # อ้างอิงผลวิเคราะห์ Cluster: 
    # Cluster 1 = Potential Starter (คะแนนต่ำสุด) -> เสี่ยงสูง (แดง)
    # Cluster 0 = Active Marketer (คะแนนปานกลาง) -> เสี่ยงปานกลาง (เหลือง)
    # Cluster 2 = Master Leader (คะแนนสูงสุด) -> เสี่ยงต่ำ (เขียว)
    # =========================================================
    
    if cluster_id == 1: # Potential Starter (แดง) -> บังคับช่วง 71-100%
        # สูตร: ฐาน 71 + (ค่าดิบ * ช่องว่าง 29)
        risk_score = 71 + (raw_prob * 29)
        
    elif cluster_id == 0: # Active Marketer (เหลือง) -> บังคับช่วง 41-70%
        # สูตร: ฐาน 41 + (ค่าดิบ * ช่องว่าง 29)
        risk_score = 41 + (raw_prob * 29)
        
    else: # Cluster 2: Master Leader (เขียว) -> บังคับช่วง 0-40%
        # สูตร: ฐาน 0 + (ค่าดิบ * ช่องว่าง 40)
        risk_score = raw_prob * 40
    ...
    # ✅✅✅ แก้ไข Mapping DNA ตรงนี้ให้สอดคล้องกัน ✅✅✅
    cluster_info = {
    ...
    # ดึงค่าตาม cluster_id ที่คำนวณได้
    dna = cluster_info.get(cluster_id, cluster_info[3])
    ...
    with col1:
        ...
        # Logic คำแนะนำ (ปรับให้ตรงกับ ID ใหม่)
    ...
    with col2:
        st.markdown(f"### 🔮 ข้อจำกัดการเข้าถึงแหล่งเงินทุน: **{risk_score:.1f}%**", unsafe_allow_html=True)
        ...
        fig = go.Figure(go.Indicator(
            ...
            value = risk_score,
            gauge = {
                'axis': {'range': [4], 'tickwidth': 1, 'tickcolor': "gray"},
                'bar': {'color': "darkblue"},
                'bgcolor': "white",
                'borderwidth': 2,
                'bordercolor': "gray",
                'steps': [
                    {'range': [5], 'color': "#99df1b"},   # เขียว (Master)
                    {'range': [5, 6], 'color': "#F9D607"},  # เหลือง (Active)
                    {'range': [4, 6], 'color': "#e74c3c"}  # แดง (Potential)
                ],
                'threshold': {
